        ...
        and was released under the MIT License. I altered in on Nov 10, 2015 to fit the needs of this site.*/?>
                <script>
					...
					<div class="join-box" id="single-webinar">
                        ...
                        <?php
                        $temp1 = strtotime("+6 hours".$webinar_date." ".$webinar_time);
                        $temp2 = get_the_title();
                        echo '<button class="blue" style="color: white; font-size: 12px !important" onclick="downloadICS(\''. $temp2 .'\', \''. datetoCal($temp1) .'\')">Add to Cal</button>';
                        ?>
					</div>

                <script>
                    var cal = ics();
                    function downloadICS(name, time){
                        var realName = 'Webinar: ' + name;
                        cal.addEvent(realName, 'Content', 'Location', time, time, time);
                        cal.download(realName);
                    }
                </script>
                <?php
            }

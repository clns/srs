...
?>
			...
			if ( $the_query->have_posts() ) {
				while ( $the_query->have_posts() ) : $the_query->the_post();
					...
					<div class="bc<?php echo get_the_ID() ?> bootcampInfoBlock" <?php echo $display ?>>
						...
						?>
						<h1 style="font-size: 26px; line-height: 34px;">Welcome
							to <?php echo $ministry_name_possessive ?>
							registration site for support raising training.</h1>
	...
	<div class="grid_12 clearfix">
		<?php if ( $the_query->post_count > 1 ): ?>
			<div id="select-container">
				...
				<select id="select-bootcamp" style="display: block; margin: 0 auto;"
				        onChange="changeBootcampDisplay(this.value)">
					<?php if ( $the_query->have_posts() ) {
						while ( $the_query->have_posts() ) : $the_query->the_post();
							...
							<option
								value="bc<?php echo get_the_ID() ?>"><?php echo get_post_meta( $post->ID, 'location_city', true ) . ', ' . get_post_meta( $post->ID, 'location_state', true ) . ' ' . $start_date . '-' . $end_date ?></option>
						<?php endwhile;
					}
					wp_reset_postdata(); // end of the loop.
					?>
		...
		<div class="bootcamp-header">
			...
			if ( $the_query->have_posts() ) {
				while ( $the_query->have_posts() ) : $the_query->the_post();
					...
					<div class="bc<?php echo get_the_ID() ?> bootcampInfoBlock" <?php echo $display ?>>
						<h2
							class="location-header"><?php echo get_post_meta( $post->ID, 'location_city', true ) . ', ' . get_post_meta( $post->ID, 'location_state', true ) . ' ' . $start_date . '-' . $end_date ?></h2>
					</div>
				<?php endwhile;
			}
			wp_reset_postdata(); // end of the loop.
			...
			if ( $the_query->have_posts() ) {
				while ( $the_query->have_posts() ) : $the_query->the_post();
					...
					<div class="bc<?php echo get_the_ID() ?> bootcampInfoBlock button-container" <?php echo $display ?>>
						<p class="button-text"><?php if ( $include_link ) {
								echo '<a href="' . get_post_meta( $post->ID, 'registration_link', true ) . '">';
							}
							echo '<span ' . $closed_style . '>Register</span>';
							if ( $include_link ) {
								echo '</a>';
							}
							echo $closed_text; ?></p>
	...
	<div class="grid_12 expanding-container col-right">
		...
		<!-- BEFORE COMING TO BOOTCAMP -->
		<h3 class="expanding-heading">Before Coming to Bootcamp<span class="expanding-icon expanding-icon-minus"></span>
		</h3>
		...
		<div class="expanding-content first-expanding-content">
			...
			if ( $the_query->have_posts() ) {
				while ( $the_query->have_posts() ) : $the_query->the_post();
					...
					$before_coming = get_post_meta( get_the_ID(), 'before_coming', true ); ?>

					<div class="bc<?php echo get_the_ID() ?> bootcampInfoBlock" <?php echo $display ?>>
						<?php if ( empty( $before_coming ) ) { ?>
							<ol>
								<li><?php echo $registration_text ?></li>
								<li><?php echo $prep_packet_text ?></li>
								<li>Receive <span style="font-style: italic;">The God Ask</span> and <span
										style="font-style: italic;">Viewpoints</span>,
		...
		<div class="expanding-content">
			...
			if ( $the_query->have_posts() ) {
				while ( $the_query->have_posts() ) : $the_query->the_post();
					...
					$about_srs_bootcamp = get_post_meta( get_the_ID(), 'before_coming', true ); ?>
					<div class="bc<?php echo get_the_ID() ?> bootcampInfoBlock" <?php echo $display ?>>
						<?php if ( empty( $about_srs_bootcamp ) ) { ?>
							<p>Since 2001, SRS has trained nearly 10,000 mission workers from over 500 ministries to be
								spiritually
								healthy,
								vision-driven, and fully funded Great Commission workers. SRS staff members raise their
								own support to
								provide
								excellent training resources, equip you to overcome obstacles in support raising, and
								help you thrive in
								ministry. </p>

							<p>SRS Bootcamp is a completely interactive and engaging workshop. It is essential that you
								complete all
								the
								Bootcamp preparation before you arrive (24-40 hours), because once you get to Bootcamp,
								you will
								synthesize and
								practice with others what you learned during your preparation work. At the Bootcamp, you
								will: </p>
							<ul>
								<li>Gain confidence in communicating the biblical foundation for living on support,
									asking others to
									invest, and
									understanding the "God Ask"
								</li>
	...
	<!-- end grid_12 expanding-container col-right -->
	</div>
	<!-- end container_24 page-franchise-participant clearfix -->
	</div>
	<!-- end page-franchise-participant -->
	<div class="clear"></div>
